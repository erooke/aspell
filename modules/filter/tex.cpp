...
private:
  enum InWhat { Name, Opt, Parm, Other, Swallow, InlineMath, DisplayMath };
  struct Command {
    InWhat in_what;
    String name;
    const char *do_check;
...
inline void TexFilter::push_command(InWhat w) { stack.push_back(Command(w)); }
...
inline bool TexFilter::process_char(FilterChar::Chr c) {
  ...
  if (prev_backslash) {
    if (c == '[') {
      push_command(DisplayMath);
      return true;
    }

    if (c == '(') {
      push_command(InlineMath);
      return true;
    }

    if (c == ')' && top.in_what == InlineMath) {
      pop_command();
      return true;
    }

    if (c == ']' && top.in_what == DisplayMath) {
      pop_command();
      return true;
    }
  }
  ...
  if (top.in_what == InlineMath || top.in_what == DisplayMath) {
    return true;
  }
...
PosibErr<bool> TexFilter::Commands::remove(ParmStr value) {
  int p1 = 0;
  while (!asc_isspace(value[p1]) && value[p1] != '\0')
    ++p1;
  String temp;
  temp.assign(value, p1);
